...
func TestDB_Put(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("D:\\golang_pro\\kv_project\\tmp", "bitcask-go-put")
	opts.DirPath = dir
	...
	for _, file := range db.olderFiles {
		file.IOManager.Close()
	}
	for _, file := range db2.olderFiles {
		file.IOManager.Close()
	}
	db.activeFile.IOManager.Close()
	db2.activeFile.IOManager.Close()
}
...
func TestDB_ListKeys(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("D:\\golang_pro\\kv_project\\tmp", "bitcask-go-list-keys")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Fold(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("D:\\golang_pro\\kv_project\\tmp", "bitcask-go-fold")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Close(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("D:\\golang_pro\\kv_project\\tmp", "bitcask-go-close")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Stat(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("D:\\golang_pro\\kv_project\\tmp", "bitcask-go-stat")
	opts.DirPath = dir
	db, err := Open(opts)
	// defer destroyDB(db)
	assert.Nil(t, err)
